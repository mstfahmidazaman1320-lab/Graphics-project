...
//float fishY = 125.0f;     // current y-position of fish
//float fishSpeed = 0.2f;   // speed of fish bounce

float fishY = 0.0f;    // fish vertical offset
//float fishSpeed = 0.5f; // fish movement speed
...
/*void drawBirds()
{
    ...
    for(int i = 0; i < NUM_BIRDS; i++)
    {
        ...
        glVertex2f(birds[i].x - 14, birds[i].y);
        glVertex2f(birds[i].x,      birds[i].y + 10);
...
void drawBirds() {
    for(int i = 0; i < NUM_BIRDS; i++) {
        glColor3f(0, 0, 0); // কালো ছোট পাখি
        glBegin(GL_LINES);
        glVertex2f(birds[i].x - 18, birds[i].y);
        glVertex2f(birds[i].x, birds[i].y + 12);
        glVertex2f(birds[i].x, birds[i].y + 12);
        glVertex2f(birds[i].x + 18, birds[i].y);
        glEnd();
...
{
    glPushMatrix();              // <-- start overall bird
    glTranslatef(x, y, 0);

    // BODY
    glColor3f(0.9f, 0.9f, 0.9f);
    drawCircle(0, 60, 18, 50);
    ...
    // HEAD + BEAK + EYE + FISH
    glPushMatrix();             // <-- push for head rotation only
    glTranslatef(0, 110, 0);   // neck top
    glRotatef(headAngle, 0, 0, 1); // rotate head + beak + fish

    // HEAD
    glColor3f(1.0f, 1.0f, 1.0f);
    drawCircle(0, 0, 12, 40);
    ...
    // FISH (attached to beak)
    drawFish(40, -3 + fishY);

    glPopMatrix();  // <-- pop head rotation
...

{
    ...
    //if (cloudPosX1 > 1200) cloudPosX1 = -500;
    //if (cloudPosX2 > 1200) cloudPosX2 = -500;
    //if (cloudPosX3 > 1200) cloudPosX3 = -500;
...
// head rotation
    ...
    if(fishY > 5 || fishY < 0) fishSpeed = -fishSpeed; // limits
      ...
      //  fishY += 0.2f * fishSpeed;
//if(fishY > 10 || fishY < -5) fishSpeed = -fishSpeed;
   ...
   /* // birds move
    ...
    if(birds[i].x > 800) birds[i].x = -20; // screen edge
}
    ...
    for(int i = 0; i < NUM_BIRDS; i++)
    {
        ...
        // screen cross করলে আবার left থেকে আসবে
        if(birds[i].x > 1250)
        {
            birds[i].x = -50;
            birds[i].y = 500 + rand() % 200;  // নতুন height
        }
    }
*/
...
/*void initBirds() {
    for(int i = 0; i < NUM_BIRDS; i++) {
        birds[i].x = rand() % 800;
        //birds[i].y = 400 + rand() % 100;
        birds[i].y = 550 + rand() % 150;  // 550 – 700

        birds[i].speed = 1.0f + (rand() % 3) * 0.5f;
    }
}*/
